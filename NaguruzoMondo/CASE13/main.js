...
let answers = ["zzz", "ZZZ","Zzz"];

let remainingAttempts = 3;

let revealedQuestions = 0;
let questionPos = [-1, -1, -1, -1];
let first_question_cand = [[25,15,21], [7, 25, 13], [19, 11, 25]];
let first_answer_cand = [31,32,30];
let questionImages = [[1,3,5],[20,23,15],[15,23,12],[26, 26, 26]];
let answerImages = [27, 28, 29, 34];
...
function saveDorosute(){
    let nowscore = 25 - revealed;

    img = get(0, 0, width, height);
    savedImages[nowscore] = img;
}

function getSavedDorosute(index) {
...
function calcNewImage(index) {
    ...
    if (questionImages.length == 4){
        let selectQ = index%5;
        if (selectQ == 4){
            selectQ = Math.floor(Math.random()*3);
        }

        questionImages.splice(line, 0, first_question_cand[selectQ]);
        answerImages.splice(line, 0, first_answer_cand[selectQ]);
    }

    if (index%5 == 4){
        return answerImages[line];
    }

    return questionImages[line][index%5] + 10000;
}
...
function make_tweet(res = 0) {
    ...
    if (res == 0) {
        tweetText = `CASE${nazoid}\n\nScore: ${score}/${grid * grid + 1} (${attempt}回目)\n`;
    }
    for (let i = 0; i < grid; i++) {
        ret = "";
        for (let j = 0; j < grid; j++) {
    ...
    for (let i = 0; i < grid; i++) {
        for (let j = 0; j < grid; j++) {
            ...
            if (showidx[index] < images.length) {
                ...
                num = showidx[index] - 10000;
                img = getSavedDorosute(num);
                if (img) {
                    image(img, j * cellWidth, i * cellHeight, cellWidth, cellHeight);
                }else{
                    image(images[33], j * cellWidth, i * cellHeight, cellWidth, cellHeight);
                }
            }
        }
    }
