...
function applyProgressFromMain() {
    ...
    if (progressParam) {
        ...
        console.log('Stage2: 開けたパネル数', revealed);
    }
}
...
function mousePressed() {
    ...
    if (clicked[row * grid + col] === 1) {  // 1に統一
        return;
    }
...
function mouseReleased() {
    ...
    if (cleared == 0 && floor(startX / cellWidth) === floor(mouseX / cellWidth) && floor(startY / cellHeight) === floor(mouseY / cellHeight)) {
        ...
        if (clicked[row * grid + col] == 0 && col >= 0 && col < grid && row >= 0 && row < grid) {
            ...
            clicked[index] = 1;  // 1に統一（generateProgressParamと一致させる）
...
function animateDestruction() {
    ...
    function updateDestruction() {
        ...
        // キャンバス範囲を徐々に狭める
        // 横方向: 0% → 20%~80% (60%の幅)
        // 縦方向: 0% → 20%~100% (80%の高さ、上から20%カット)
        const targetClipX = width * 0.2;
        const targetClipY = height * 0.2;
        const targetClipWidth = width * 0.6;  // 80% - 20% = 60%
        const targetClipHeight = height * 0.8; // 100% - 20% = 80%
        
        // イージング関数（スムーズな減速）
        const easeOut = 1 - Math.pow(1 - progress, 3);
        
        canvasClipX = lerp(0, targetClipX, easeOut);
        canvasClipY = lerp(0, targetClipY, easeOut);
        canvasClipWidth = lerp(width, targetClipWidth, easeOut);
        canvasClipHeight = lerp(height, targetClipHeight, easeOut);
...
function showInteractionHint() {
    ...
    hint.textContent = 'さあ、謎解きを始めましょう';
...
function addNextChapterButton(overlay) {
    ...
    nextButton.href = `./stageL.html${progressParam}`;
    nextButton.textContent = '最終章へ';
