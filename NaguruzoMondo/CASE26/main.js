...
function drawSpecial(n1, n2) {
    // 特殊効果の描画（必要に応じて実装）
    // 白い大きな文字で左上にn1, 右上にn2を表示
    // 左上の数字はゲーム画面の横幅0-50%, 縦幅0%-60%の範囲に表示する
    textFont("Impact, 'Arial Black', Arial, sans-serif");
    textSize(width * 0.5);
    fill(255, 255, 255);
    textAlign(CENTER, TOP);
    text(n1, width * 0.25, height * 0.1);
    textAlign(CENTER, TOP);
    text(n2, width * 0.75, height * 0.1);
...
if (submitButton) {
    submitButton.addEventListener('click', () => {
        ...
        if (answers.includes(answerInput)) {
            ...
            if (revealed == 25){
                window.showCaseMessage(`ちがいます。\nヒント 12個あるものについて調べてみると、一般的なビンゴカードで全開けしたときに成立するビンゴの列数がヒットします。`);
            }else{
                window.showCaseMessage(`ちがいます`);
            }
