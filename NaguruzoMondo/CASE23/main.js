...
let answers = ["えこ","エコ", "カリホルニトラム", "カリフォルニトラム", "かりほるにとらむ", "かりふぉるにとらむ", "カリホルニトラム座", "カリフォルニトラムざ"];
let HINT = "ヒント: 矢印の法則は「ローマ字にして逆から読む」です！";
...
function drawArea() {
    ...
    // 動かないパネル（クリック済み かつ idle）を先に描画
    for (let panel of panels) {
        if (clicked[panel.gridIndex] && panel.animationState === 'idle') {
            panel.display();
        }
    }
    // 動くパネル（未クリック or アニメーション中）を後で描画
    for (let panel of panels) {
        if (!(clicked[panel.gridIndex] && panel.animationState === 'idle')) {
            panel.display();
        }
