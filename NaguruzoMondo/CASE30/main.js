...
let answers = ["じびか","耳鼻科"];
let hintMessage = "ちがいます。「一富士」「二鷹」「三なすび」みたいなことをしましょう。";
...
function calcNewImage(index) {
    if ([0,1,2,3,4,5,9,10, 14,15,19,20,21,22,23,24].includes(index)) {
        // 配列remainImagesの要素をランダムに選ぶ(0ならやり直し)
        let v=0;
        while (remainImages[0] > 0 || remainImages[1] > 0 || remainImages[2] > 0) {
            if (v>100)return 3;
            v++;
            let rd = floor(random(0,3));
            if (remainImages[rd] > 0) {
                remainImages[rd]--;
                return 26 + rd + 1;
            }
        }
